...
>
      ...
      <header className="doc-header"><div className="doc-company"><img src="/new-logo.png" alt="Shri Venkatesh Plumbing Work Washim logo" /><div><h1>SHRI VENKATESH PLUMBING WORK WASHIM</h1><p>PRO. {settings?.proprietor_name || 'HARISH S PHUSE'}</p><p>SHRI VENKATESH PLUMBING WORK WASHIM</p><p>MOB. 9623199934 / 8329847776</p></div></div><div className="doc-type-badge"><strong>{type === 'invoice' ? 'INVOICE' : 'ESTIMATE'}</strong><span>{number}</span>{isPaid && <em className="paid-tag">PAID</em>}{isPartial && <em className="partial-tag">PARTIAL</em>}</div></header>
...
function EstimatePreview({ data, settings }: { data: EstimatePreviewData; settings: SettingsData | null }) {
  ...
  return <div className="preview-shell"><div className="preview-actions"><button className="button secondary" onClick={() => window.print()}><Printer size={16} /> Print</button></div>
    <div className="a4-sheet estimate-sheet" id="document-print">
      <header className="doc-header"><div className="doc-company"><img src="/new-logo.png" alt="Shri Venkatesh Plumbing Work Washim logo" /><div><h1>SHRI VENKATESH PLUMBING WORK WASHIM</h1><p>PRO. {'HARISH S PHUSE'}</p><p>SHRI VENKATESH PLUMBING WORK WASHIM</p><p>MOB. 9623199934 / 8329847776</p></div></div><div className="doc-type-badge"><strong>ESTIMATE</strong><span>{estimate.estimate_number}</span></div></header>
